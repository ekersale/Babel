...
bool					XMLParser::openFile(std::string filename)
{
  ...
  QDomDocument				doc(stdStringToQString(filename));
  QFile					xmldoc(stdStringToQString(filename));
  QString				error;
  int					errorLine;
  int					errorColumn;
  ...
  while (!node.isNull())
    {
      ...
      elem = node.toElement();
      if (!elem.isNull() && elem.tagName() == "contacts")
	std::cout << elem.text().toStdString() << std::endl;
								    ...
								    std::string child, std::string value)
{
  ...
  std::fstream				readFile(filename.c_str(), std::fstream::in);
  std::string				line;
